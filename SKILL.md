...
├── ChangeLog.md          # 人格特质更新日志（单个 MD 文件，记录更新情况及时间戳）
└── temp/                 # 临时目录（运行时自动创建）
    ├── USER.md           # 预览文件（待用户审核）
...
- `ChangeLog.md` 是**单个 Markdown 文件**（不是文件夹），在配置阶段自动创建
- `ChangeLog.md` 用途：记录人格特质的更新情况及时间戳（每次更新追加内容）
...
├── SKILL.md
├── clawhub.yaml
├── ChangeLog.md          ← 单个 MD 文件（记录人格特质更新历史）
│   # 内容示例：
│   ## 2026-03-13-2030
│   - 沟通风格：直接高效 (饱和度 75%, 置信度 80%)
│   - 决策风格：行动导向 (饱和度 68%, 置信度 72%)
│   ## 2026-03-14-0230
│   - 沟通风格：直接高效 (饱和度 78%, 置信度 85%)
│   ...
└── temp/
    ├── USER.md           ← 预览文件（用户未确认）
    └── last_analysis.json ← 时间戳
...
> 
> **定时任务（日常更新）**：
> - 读取 `temp/last_analysis.json` 获取**上次分析时间戳**
> - 只加载 **从上次分析到现在** 的语料（约 12 小时）
> - 分析完成后立即更新时间戳
> 
> **定时任务语料区间示例**：
> - 凌晨 2:30 分析：加载 昨天 14:30 → 今天 2:30 的语料
> - 下午 14:30 分析：加载 今天 2:30 → 14:30 的语料
> - 这样每个时间段只分析一次，不会重复！
...
if last_analysis_file.exists():
    # 定时任务：读取上次分析时间，只加载新增语料（约 12 小时）
    with open(last_analysis_file, 'r', encoding='utf-8') as f:
        last_time = datetime.fromisoformat(json.load(f)['timestamp'])
else:
    # 首次运行：加载所有历史对话（不限制时间，建立完整人格画像）
    last_time = datetime.min  # 加载所有历史消息
    print("ℹ️  首次运行，加载所有历史对话语料...")
...
# 首次运行：不限制消息数量（充分利用历史对话建立人格模型）
# 定时任务：限制 50 条（避免单次分析过多，每次只分析 12 小时新增语料）
is_first_run = not last_analysis_file.exists()
if not is_first_run and len(messages) > 50:
    messages = messages[-50:]
    print(f"⚠️ 定时任务：消息过多，只处理最近 50 条")
elif is_first_run:
    print(f"✅ 首次运行：加载全部 {len(messages)} 条历史语料")
...
- ✅ **语料加载逻辑**：
  - 首次运行：加载所有历史对话（不限制时间，建立完整人格画像）
  - 定时任务：读取 `temp/last_analysis.json`，只加载上次分析到现在的语料（避免重复）
- ✅ **定时任务语料区间**：凌晨 2:30 分析（昨日 14:30→今日 2:30），下午 14:30 分析（今日 2:30→14:30）
...
**文件位置**：`skills/i-am/ChangeLog.md`

**文件格式**：单个 Markdown 文件（不是文件夹）

**用途**：记录人格特质的更新历史（追加式日志）

### 更新记录格式

```markdown
# i-am Skill ChangeLog
...
**更新时间**: 2026-03-14-0230

**人格特质**:
- 沟通风格：直接高效 (饱和度：75%, 置信度：80%)
- 决策风格：行动导向 (饱和度：68%, 置信度：72%)
- 技术取向：实用主义 (饱和度：52%, 置信度：60%)

---

### 2026-03-13-2030

**更新时间**: 2026-03-13-2030

**人格特质**:
- 沟通风格：直接高效 (饱和度：70%, 置信度：75%)
- 决策风格：行动导向 (饱和度：65%, 置信度：67%)
...
```bash
# 查看完整更新历史
cat skills/i-am/ChangeLog.md
```

**对比最近两次更新**：
```bash
# 手动对比 ChangeLog.md 中的不同章节
# 或使用文本编辑器查看变更
```

**撤回更改**：
```bash
# 直接编辑 USER.md，删除或修改人格特质章节
# 或从 ChangeLog.md 复制之前的版本到 USER.md
```
...
AI: （执行步骤 1-4，按汇报规范格式输出）
    
    📊 i-am Skill 执行进展汇报
    
    ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    [✅] 语料收集区间｜2026-03-13-1430～2026-03-14-0230
    [✅] 语料收集数量｜共收集到 15 条有效语料
    [✅] 技能运行情况｜共涌现 12 个标签，聚类为 6 个类别，识别出 4 个特质（新增 1 个特质）
    ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
- ✅ 用户确认后才更新 USER.md
- ✅ 预览内容直接在消息中呈现（不依赖文件发送）
- ✅ 每次变更自动记录到 ChangeLog.md（单个 MD 文件）
